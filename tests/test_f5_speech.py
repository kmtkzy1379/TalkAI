...
def t_stale_recency_deixis() -> bool:
    ...
    return (
        ...
        # D1 案2 で意味を反転させた（2026-07-30）: 復元会話を注入しなくなったので
        # 「注入窓にユーザ発話が無い」は起動直後の**通常状態**になった。この時「さっき」は
        # 指す先が文脈に存在しない＝定義上誤り（RAG の種は `[31日前]` と明示ラベル付き）。
        # 旧仕様（材料なし→False）のままだと ②-6 が恒久 no-op になる（実測: 復元あり16/17
        # 発火 → 復元なし 0/17）。材料が無いこと自体を根拠にする。
        and f("さっきの件", [], now) is True
    )
