    ...
    def _build_system(self, *, rag_chunks, last_feedback, vision, speech_decision_reason, now,
        ...
        if tools_active:
            parts.append(
                ...
                "「5分後に〜」などの予約・後回しの作業は create_task で記録すると、時間が来たら自動実行される。"
                "予約の取り消しは cancel_task を**直接1回**呼ぶ（一覧確認は不要・ID 省略で直近の予約が消える）。\n"
                "複数段階の作業や調べ物（『〜を調べてまとめて』等）は **delegate_task** で賢いタスク担当に"
                "任せられる（イブが自律的に『これは任せよう』と判断して呼んでよい）。任せたら「やっとくね」と"
                "一言添えれば、達成後に結果が後で届く。"
            )
        return "\n\n".join(parts)
