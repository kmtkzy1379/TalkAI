...
Tier-1 決定論テスト **155件が2回連続 PASS**。未実装: SurpriseBus(多生産者集約・VLM時)・(b)自己懐疑(タスク隣接)・VLM・Call-Function・YouTube・UI・配線層PORT(vts/run/launcher/app)。**中核原理 surprise は生産者(F4 `PredictionState`)+消費(F5 `should_speak`)が両方稼働**。**surprise は数値で発話を絶対決定せず「指標」として発話判定LLMが感情/内容と総合判断**（Fix2 裁定・HI/LO 強制ゲート撤廃）。T2 は「surprise が必須引数として判定に効く配線」。多生産者 SurpriseBus は VLM 時に。
