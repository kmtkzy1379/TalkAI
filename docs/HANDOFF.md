...
- 未実装: (b)文脈不整合の自己懐疑(タスク管理隣接) / 多生産者 SurpriseBus(VLM は現状 PredictionState 第2生産者で直結) / Call-Function(task/search/screen-op) / YouTube / UI / 配線層PORT(vts/run/launcher/app) / VLM の Gemini Live ストリーム mode(将来)。
  ...
  - **実機検証済（2026-06-25・gpt-4o + gemini-2.5-flash・`tools/f6_realtest.py` で5+1シナリオ×3回）**: 画面内容の読み取り・差分実況(A8)・会話と画面の混ぜ方(無関係画面に引っ張られない)・タイミング近接で破綻なし・backpressure 自己回復、を確認。実 Gemini への画像送信(litellm `image_url`→`inline_data`)も通る。**実機で発見し修正済**: ①応答LLMの捏造(未知語に偽定義)→ SPEECH_STYLE に正直さ節(聞き取れない/未知語は作文せず画面fallback/一度だけ聞き返す) ②静止画面で内容を失い「見えない」誤答→ narrator に「変化無くても主要表示を必ず一言」 ③遅延スパイク(max9-11s)での古い画面参照→ `VisionState.fresh_vision` + `VLM_VISION_TTL_SEC=7s`(古い画面は応答/判定に渡さない=明らか過去を構造的に参照させない・実機 max age 5.6s)。軽量化(解像度/枚数)は遅延に効かずOCR悪化で撤回(遅延は Gemini API 固有)。
  - **F6 残課題（軽微・繰越）**: アプリ/ファイル名の OCR 誤読（内容は正確・Eve は「〜のような/らしき」と hedge＝**及第点**）→ **タスク管理/画面操作フェーズでウィンドウ/アプリ情報＋キーボード/マウス操作を認識させる時に改善見込み**。VLM レイテンシ~5s は Gemini API 固有(TTL で安全吸収)。自発発話頻度の正式調整は RAG 充実後(指標主義のまま)。
  - **マージ状況**: F6 はユーザの**自己実機テスト後に main マージ**予定（VLM 既定 off なので main 挙動は不変）。
...
- テスト: **Tier-1 12ファイル 200件 2回連続 PASS**（API不要・決定論）。flaky なし。F6 は `tests/test_f6_vlm.py`（37件・backpressure/single-flight/A1/A11/鮮度TTL ⭐含む）。F5 は `tests/test_f5_speech.py`（29件）。実機評価は `tools/f6_realtest.py`（VLM 通し）/ `tools/f5_model_eval.py`（応答自然さ）/ `tools/f6_latency_ab.py`（Flash 遅延 A/B）。
